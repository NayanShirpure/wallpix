...
export async function GET(request: NextRequest) {
  ...
  if (!pexelsApiKey) {
    console.error('[API/PEXELS/SEARCH] PEXELS_API_KEY is not set on the server.');
  ...
  try {
    ...
    if (!pexelsResponse.ok) {
      ...
      console.error(`[API/PEXELS/SEARCH] Pexels API error: ${pexelsResponse.status}`, errorBody.substring(0, 500));
